...
def test_genomic_module_contributes_rail_and_script():
    ...
    assert 'id="gen-ne-skygrid"' not in html and 'id="gen-ne-exp"' not in html   # no Ne model toggles
    ...
    # Rail order: tree → Ne → cases/genomes → correlation.
    assert html.index('id="gen-tree-card"') < html.index('id="gen-ne-card"')
    assert html.index('id="gen-ne-card"') < html.index('id="gen-dist-card"')
    assert html.index('id="gen-dist-card"') < html.index('id="gen-corr-card"')
